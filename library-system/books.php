...
// Fetch books with current quantity
$stmt = $conn->prepare("SELECT id, category, title, quantity FROM books ORDER BY category, title");
$stmt->execute();
        ...
        <tbody>
         <?php while ($book = $books->fetch_assoc()): ?>
            <tr>
                ...
                <td>
                    <!-- Borrow button disabled if out of stock -->
                    <form action="borrow.php" method="POST" style="display:inline;">
                        <input type="hidden" name="book_id" value="<?= $book['id'] ?>">
                        <button type="submit" class="borrow-btn" <?= $book['quantity'] <= 0 ? 'disabled' : '' ?>>
                            Borrow
                        </button>
                    </form>

                    <!-- Optional return button (if you manage returns) -->
                    <form action="return.php" method="POST" style="display:inline;">
                        <input type="hidden" name="book_id" value="<?= $book['id'] ?>">
                        <button type="submit" class="borrow-btn" style="background:#dc3545;color:white;">
                            Return
                        </button>
                    </form>
                </td>
